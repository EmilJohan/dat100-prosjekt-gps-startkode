...
public class GPSComputer {
	...
	public int totalTime() {
		int fulltime;
		fulltime =  gpspoints[gpspoints.length - 1].getTime() - gpspoints[0].getTime();
		return fulltime;
	...
	public double[] speeds() {
		...
		for (int i = 1; i <= gpspoints.length - 1; i++) {
			speed[i-1] = ((GPSUtils.distance(gpspoints[i-1], gpspoints[i])) / (gpspoints[i].getTime()-(gpspoints[i-1].getTime()))*tidmodifier);
			}	
		return speed;
	}
	...
	public double totalKcal(double weight) {
		double totalkcal;
		int time = totalTime();
		double speed = averageSpeed()/tidmodifier;
